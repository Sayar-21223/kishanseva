 <?php
        ...
        if (isset($_SESSION['phonenumber'])) {
            ...
            if ($run_query) {
                while ($row_cat = mysqli_fetch_array($run_query)) {
                    ...
                    $buyer_name = '<span style="color: #ffff84;">Hello, ' . $buyer_name . '</span>';
                }
                echo "<div class='text-success  logins mx-1 ml-5'>$buyer_name</div>";
            }
        } else {
            echo "<a style='color:#ffff84;' href='../auth/BuyerLogin.php' class=' mx-5'>Login</a>";
        }
    }
    ...
    function getFarmerUsername()
    {
        if (isset($_SESSION['phonenumber'])) {
            ...
            echo "<label><a href = '../auth/FarmerLogin.php'>Login/Sign up</a></label>";
        }
    }
    ...
    {
        global $con;
    $query = "SELECT * FROM products WHERE product_cat = 2 AND product_image != '' GROUP BY product_image ORDER BY RAND() LIMIT 0,4";
    $run_query = mysqli_query($con, $query);
    while ($rows = mysqli_fetch_array($run_query)) {
        $product_id = $rows['product_id'];
        $product_title = $rows['product_title'];
...
{
    global $con;
    $query = "SELECT * FROM products WHERE product_cat = 3 AND product_image != '' GROUP BY product_image ORDER BY RAND() LIMIT 0,4";
    $run_query = mysqli_query($con, $query);
    while ($rows = mysqli_fetch_array($run_query)) {
        $product_id = $rows['product_id'];
        $product_title = $rows['product_title'];
...
//         echo "<p>Name: " . $row['applicant_name'] . ", Phone: " . $row['applicant_phone'] . ", Email: " . $row['applicant_email'] . ", Amount: " . $row['loan_amount'] . "</p>";
//     }
// }

function getLoanApplications($phoneNumber)
{
    global $con; // Assuming $con is your database connection object

    // Prepare the SQL query to select loan applications for the given user
    $query = "SELECT * FROM loan_applications WHERE applicant_phone = '$phoneNumber'";

    // Execute the query
    $result = mysqli_query($con, $query);

    // Check if there are any loan applications for the user
    if (mysqli_num_rows($result) > 0) {
        // Loop through each loan application and display information
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<p>";
            echo "Date: " . date('Y-m-d', strtotime($row['application_date'])) . "<br>";
            echo "Name: " . $row['applicant_name'] . "<br>";
            echo "Phone: " . $row['applicant_phone'] . "<br>";
            echo "Email: " . $row['applicant_email'] . "<br>";
            echo "Address: " . $row['address'] . "<br>";
            echo "Account No: " . $row['account_no'] . "<br>";
            echo "Amount: " . floor($row['loan_amount']) . "<br>";
            echo "Year: " . $row['year'] . "<br>";
            echo "Final Ammount: " . floor($row['loan_amount'] * ((1 + 0.03)** $row['year'])) . "<br>";
            echo "Final Date: " . date('Y-m-d', strtotime($row['application_date'] . ' + ' . $row['year'] . ' years')) . "<br>";
            echo "</p>";
        }
    } else {
        // If there are no loan applications, display a message
        echo "No loan applications found for this user.";
    }
}
